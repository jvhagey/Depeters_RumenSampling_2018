...
I didn't demultiplex the kits yet so I will need to do this before publishing

```{r checking out kit sequences, eval=FALSE, include=FALSE}
...
Looks like there is no singletons, doubletons, and tripletons which is a bit atypical. This could be due to DADA2 pipeline and removing superfiluous reads (i.e. chimeras and read error). It's also probably a product of having 68 samples and getting deep sequencing. 

```{r}
#Checking which samples have fewer than 2000 reads
#function returns sample name and its column number
which(!rowSums(otu_table(ps)) > 2000)
```
...
Looks like most fecal samples pull away from the other samples on the first axis.Solid and Liquid strained samples move higher on the 2nd axis, but this difference is half that of the differences between fecal samples and all other samples. Overall, it appears that there is 3 "clusters".
...
Here there is a similar pattern with 3 clusters forming and fecal samples moved away from other sample types.
...
Again Firmicutes obsure much of the graph and when they are removed the data suggest that most of the varitation in both axsis comes from Bacteroidetes ASVs. 
...
res.taxa <- list(res_FvsGS.taxa,res_FvsS.taxa,res_FvsST.taxa,res_FvsLS.taxa,res_FvsLU.taxa,res_SvsST.taxa,res_SvsGS.taxa,res_SvsLS.taxa,
                 res_SvsLU.taxa,res_STvsGS.taxa,res_STvsLS.taxa,res_STvsLU.taxa,res_GSvsLS.taxa,res_GSvsLU.taxa,res_LSvsLU.taxa)
TopOTU <- lapply(res.taxa, function(res.taxa) res.taxa[1,10:13])
```
...
plotMA(res[[1]],xlab="Mean of Normalized Counts", ylab="Log Fold Change", ylim=c(-23,6), cex.main=1.5,cex.lab=1.25)
with(res[[1]][topOTU[[1]], ], {
  points(baseMean, log2FoldChange, col="dodgerblue", cex=2, lwd=2)
  text(baseMean, -15, "Christensenellaceae \n R-7 group", cex=1.5, font= 3, pos=2, col="dodgerblue")
})
plotMA(res[[2]],xlab="Mean of Normalized Counts", ylab="Log Fold Change", ylim=c(-6,6),cex.main=1.5,cex.lab=1.25)
with(res[[2]][topOTU[[2]], ], {
  points(baseMean, log2FoldChange, col="dodgerblue", cex=2, lwd=2)
  text(baseMean, 5.5, "Christensenellaceae \n R-7 group", cex=1.5, font= 3, pos=2, col="dodgerblue")
})
plotMA(res[[3]], xlab="Mean of Normalized Counts", ylab="Log Fold Change", ylim=c(-10,10), cex.main=1.5,cex.lab=1.25)
with(res[[3]][topOTU[[3]], ], {
  points(baseMean, log2FoldChange, col="dodgerblue", cex=2, lwd=2)
  text(baseMean, -9.5, "Christensenellaceae \n R-7 group", cex=1.5, font= 3, pos=2, col="dodgerblue")
})
plotMA(res[[4]], xlab="Mean of Normalized Counts", ylab="Log Fold Change", ylim=c(-10,10), cex.main=1.5,cex.lab=1.25)
with(res[[4]][topOTU[[4]], ], {
  points(baseMean, log2FoldChange, col="dodgerblue", cex=2, lwd=2)
  text(baseMean, -9.5, "Christensenellaceae \n R-7 group", cex=1.5, font= 3, pos=2, col="dodgerblue")
})
plotMA(res[[5]], xlab="Mean of Normalized Counts", ylab="Log Fold Change", ylim=c(-10,10), cex.main=1.5,cex.lab=1.25)
with(res[[5]][topOTU[[5]], ], {
  points(baseMean, log2FoldChange, col="dodgerblue", cex=2, lwd=2)
  text(baseMean, -9.5, "Christensenellaceae \n R-7 group", cex=1.5, font= 3, pos=2, col="dodgerblue")
})
plotMA(res[[6]], xlab="Mean of Normalized Counts", ylab="Log Fold Change", ylim=c(-10.5,10.5), cex.main=1.5,cex.lab=1.25)
with(res[[6]][topOTU[[6]], ], {
  points(baseMean, log2FoldChange, col="dodgerblue", cex=2, lwd=2)
  text(baseMean, -10, "Christensenellaceae", cex=1.5, font= 3, pos=2, col="dodgerblue")
})
plotMA(res[[7]], xlab="Mean of Normalized Counts", ylab="Log Fold Change", ylim=c(-10,10), cex.main=1.5,cex.lab=1.25)
with(res[[7]][topOTU[[7]], ], {
  points(baseMean, log2FoldChange, col="dodgerblue", cex=2, lwd=2)
  text(baseMean, -9.5, "Christensenellaceae", cex=1.5, font= 3, pos=2, col="dodgerblue")
})
plotMA(res[[8]], xlab="Mean of Normalized Counts", ylab="Log Fold Change", ylim=c(-7,7), cex.main=1.5,cex.lab=1.25)
with(res[[8]][topOTU[8]], ], {
  points(baseMean, log2FoldChange, col="dodgerblue", cex=2, lwd=2)
  text(baseMean, 5.5, "Christensenellaceae", cex=1.5, font= 3, pos=3, col="dodgerblue")
})
plotMA(res[[9]], xlab="Mean of Normalized Counts", ylab="Log Fold Change", ylim=c(-7,7), cex.main=1.5,cex.lab=1.25)
with(res[[9]][topOTU[[9]], ], {
  points(baseMean, log2FoldChange, col="dodgerblue", cex=2, lwd=2)
  text(baseMean, 5.5, "Christensenellaceae", cex=1.5, font= 3, pos=3, col="dodgerblue")
})
plotMA(res[[10]], xlab="Mean of Normalized Counts", ylab="Log Fold Change", ylim=c(-7,7), cex.main=1.5,cex.lab=1.25)
with(res[[10]][topOTU[[10]], ], {
  points(baseMean, log2FoldChange, col="dodgerblue", cex=2, lwd=2)
  text(baseMean, 5.5, "Fibrobacteraceae", cex=1.5, font= 3, pos=3, col="dodgerblue")
})
plotMA(res[[11]], xlab="Mean of Normalized Counts", ylab="Log Fold Change", ylim=c(-6,6), cex.main=1.5,cex.lab=1.25)
with(res[[11]][topOTU[[11]], ], {
  points(baseMean, log2FoldChange, col="dodgerblue", cex=2, lwd=2)
  text(baseMean, 4, "Christensenellaceae", cex=1.4, font= 3, pos=4, col="dodgerblue")
})
plotMA(res[[12]], xlab="Mean of Normalized Counts", ylab="Log Fold Change", ylim=c(-6.5,6.5), cex.main=1.5,cex.lab=1.25)
with(res[[12]][topOTU[[12]], ], {
  points(baseMean, log2FoldChange, col="dodgerblue", cex=2, lwd=2)
  text(baseMean, 5, "vadinBE97", cex=1.4, font= 3, pos=3, col="dodgerblue")
})
plotMA(res[[13]], xlab="Mean of Normalized Counts", ylab="Log Fold Change", ylim=c(-6.5,6.5), cex.main=1.5,cex.lab=1.25)
with(res[[13]][topOTU[[13]], ], {
  points(baseMean, log2FoldChange, col="dodgerblue", cex=2, lwd=2)
  text(baseMean, 5.5, "Christensenellaceae", cex=1.5, font= 3, pos=2, col="dodgerblue")
})
plotMA(res[[14]], xlab="Mean of Normalized Counts", ylab="Log Fold Change", ylim=c(-6,6), cex.main=1.5,cex.lab=1.25)
with(res[[14]][topOTU[[14]], ], {
  points(baseMean, log2FoldChange, col="dodgerblue", cex=2, lwd=2)
  text(baseMean, -5.5, "vadinBE97", cex=1.5, font= 3, pos=2, col="dodgerblue")
})
plotMA(res[[15]], xlab="Mean of Normalized Counts", ylab="Log Fold Change", ylim=c(-6.5,6.5), cex.main=1.5,cex.lab=1.25)
with(res[[15]][topOTU[[15]], ], {
  points(baseMean, log2FoldChange, col="dodgerblue", cex=2, lwd=2)
  text(baseMean, 5.5, "Christensenellaceae", cex=1.5, font= 3, pos=2, col="dodgerblue")
})
```
...
When comparing rumen to fecal samples betta() estimates that the mean species-level diversity in feces is 2709 "species". It estimates that the diversity in the rumen is significantly different from the feces. 
...
When you break the rumen samples up into different sample types betta() estimates the mean species-level diversity in feces is 2632 "species". Stomach Tube, Liquid strained and unstrained samples have significantly higher "species" (on average 3829, 2141 and 2520 species respectively). However, solid samples from the rumen have a lower number of "species", but it wasn't significant compared to fecal samples.
...
```
DivNet is a new tool that allows you to estimate estimate Shannon diversity in the presence of an ecological/microbial network! It also adjusts for different sequencing depths. 
```{r}
#Running DivNet
...
```{r}
#Pull shannon out individually:
dv_water$shannon %>%
  summary %>%
  add_column("Sample_Type" = ps %>% otu_table %>% sample_names)

#Plotting
plot(c$shannon, ps, col = "Sample_Type")
```

```{r}
dv_ps_st <- ps %>%
  divnet(X = "Sample_Type", ncores = 1)

plot(dv_ps_st$shannon, water, col = "Sample_Type")
```
